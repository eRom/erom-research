...
| `scripts/agy_scratch.py` | Lance UN `agy --print` dans un scratch dir puis déplace les sorties. Évite les snapshots de repo et les rejets de sandbox. Défaut `--model "Gemini 3.8 Flash (High)"`. |
...
| `scripts/grok-deep` | CLI Bun autonome. `run` (bloquant), `start` (détaché), `status`, `list`. Défaut out-dir `~/.claude/erom-store/researchs`. Mode 100755. |
