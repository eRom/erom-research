...
3. Spawn le subagent `notebook-creator` (tool Agent, `subagent_type: "deep-research:notebook-creator"` - il est `background: true`, la task-notification arrive toute seule). La mission fournit : `<sujet>` et `REPORT_PATH=<OUT>` (littéral, absolu). Le subagent construit lui-même la requête deep search riche et écrit le rapport final à REPORT_PATH.
...
- Jamais d'auto-fire depuis un brainstorming : proposer `/deep-research-nlm`, Romain décide.
