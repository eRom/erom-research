...
Deep research par le moteur Grok, asynchrone. Contrairement à `deep-gemini` (Claude construit matrice + angles et pilote les rounds), ici la méthodologie est interne au workflow builtin de Grok (plan borné → recherche → vérification adversariale → rapport) : on délègue tout, on récupère un rapport cité. La force du duo : `deep-gemini` quand tu veux contrôler le plan de recherche, `deep-grok` quand tu veux un second moteur indépendant sans toucher au quota Google.
...
- Jamais d'auto-fire depuis un brainstorming : proposer `/erom-research:deep-grok`, Romain décide.
