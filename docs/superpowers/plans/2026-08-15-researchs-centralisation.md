...
**Goal:** Les 4 moteurs du plugin erom-research écrivent leurs rapports dans le store central `~/.claude/erom-store/researchs/` (plat, daté, frontmatter canonique avec `project:`) au lieu de `docs/research/<moteur>/` du projet courant.
...
- Chemin central canonique, partout identique : `$HOME/.claude/erom-store/researchs` (dans les scripts TS : `path.join(os.homedir(), ".claude", "erom-store", "researchs")`).
...
- Ne PAS committer dans `~/.claude` (repo distinct, géré par le nightly). Seul fichier créé hors repo : `~/.claude/erom-store/researchs/.gitignore` (Task 3), plus des fixtures jetables.
...
c) Défaut out-dir. Edit : old_string `  if (!opts.outDir) opts.outDir = path.join(process.cwd(), "docs", "research", "grok");` , new_string `  if (!opts.outDir) opts.outDir = path.join(os.homedir(), ".claude", "erom-store", "researchs");`.
...
**Files:**
- Create: `~/.claude/erom-store/researchs/.gitignore` (hors repo, non committé ici : le nightly de `~/.claude` s'en charge)
...
Écrire `~/.claude/erom-store/researchs/.gitignore` avec ce contenu exact :
...
```
Sauve dans ~/.claude/erom-store/researchs/."
```
...
git -C ~/.claude status --porcelain -- erom-store/researchs/
```
...
a) old_string : `"<CLI>" run "<sujet>" --out-dir "$(pwd)/docs/research/grok" [--budget N]`
   new_string : `"<CLI>" run "<sujet>" --out-dir "$HOME/.claude/erom-store/researchs" --project "$(basename "$(pwd)")" [--budget N]`

b) old_string : `"<CLI>" status --latest --out-dir "$(pwd)/docs/research/grok"`
   new_string : `"<CLI>" status --latest --out-dir "$HOME/.claude/erom-store/researchs"`

c) old_string : `"<CLI>" list --out-dir "$(pwd)/docs/research/grok"`
   new_string : `"<CLI>" list --out-dir "$HOME/.claude/erom-store/researchs"`
...
```
Read du rapport au chemin `report_path` du status.json (absolu, sous ~/.claude/erom-store/researchs/)
```

- [x] **Step 3: Description**

Edit : old_string : `rapport avec coverage explicite sauvé dans docs/research/grok/.`
new_string : `rapport avec coverage explicite sauvé dans ~/.claude/erom-store/researchs/.`

- [x] **Step 4: Vérification list sur le store réel**

Run: `plugin/scripts/grok-deep list --out-dir "$HOME/.claude/erom-store/researchs"; echo "exit=$?"`
Expected: `Aucun run sous /Users/recarnot/.claude/erom-store/researchs/.runs` (ou la liste des runs si des fixtures traînent) et `exit=0` : la commande de la skill est valide contre le store central sans lancer de run (zéro quota).
...
a) Edit : old_string : `ls docs/research/nlm/ 2>/dev/null`
   new_string : `grep -l "engine: notebooklm" "$HOME"/.claude/erom-store/researchs/*.md 2>/dev/null`

b) Edit : old_string : `Sauve dans docs/research/nlm/."`
   new_string : `Sauve dans ~/.claude/erom-store/researchs/."`
...
b) Edit : old_string : `Rapports cités sauvés dans docs/research/<moteur>/.` , new_string : `Rapports cités centralisés dans ~/.claude/erom-store/researchs/ (frontmatter avec projet d'origine).`.
